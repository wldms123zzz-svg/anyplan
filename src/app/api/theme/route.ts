...
4. ⚠️ [최우선 규칙] 극강의 랜덤성 (1000+ 경우의 수): 내가 방금 말한 예시들을 **절대 똑같이 반복 출력하지 마!** 현재 시스템 시간(${new Date().toISOString()})을 무작위 시드(Seed)로 삼아서, 너의 내면에 있는 1000가지 이상의 이색 외출/데이트 데이터베이스 중 주사위를 굴려 매번 완벽하게 다른 카테고리(스포츠, 힐링, 이색 체험, 병맛 대결, 감성 투어 등)를 하나 뽑아내. 중복 코스, 뻔한 코스 절대 금지.
5. 디테일: 코스 이름만 던지지 말고, 왜 이 코스가 이 컨디션에 완벽한지 생생하게 묘사해. ${focusPoint}
