...
export async function POST(req: Request) {
  ...
  try {
    ...
    const prompt = `
      당신은 대한민국 최고의 '로컬 힙스터 데이트 플래너'입니다.
      
      [미션: 3단계 시네마틱 저니]
      각 단계는 서로 다른 성격의 활동으로 구성되어야 하며, 장소와 행동이 유기적으로 연결되어야 합니다.
      (예: DDP 푸드트럭 -> LED 장미 정원 인생샷 -> 청계천 산책하며 도란도란 이야기)

      [활동 내용 구성 가이드]
      - 01 체험(Experience): 장소 고유의 힙한 활동 (예: 장터 음식 맛보기, 전시 관람)
      - 02 분위기(Atmosphere): 시각적/감성적 포인트에서 인생샷 남기기 또는 몰입형 순간
      - 03 연결(Connection): 근처 조용한 곳으로 이동해 나누는 로맨틱한 산책과 깊은 대화

      [스타일 가이드]
      - 문장은 아주 **간결하고 가독성 좋게** 작성하세요 (활동당 최대 2문장).
      - '커플', '연인', '데이트' 단어 절대 사용 금지. '우리', '함께' 등으로 표현.
      - **Random Twist (매우 중요)**: 가위바위보 게임(지는 사람이 음료 사기 등), 서로에게 전하고 싶은 말 편지 쓰기, 오늘 찍은 사진들 중 최고를 골라 스크랩하기, 서로의 첫인상을 묘사해주기 등 아주 **센스 있고 창의적인** 미션을 제안하세요.
      ...
      반드시 아래 JSON 형식으로 응답하세요:
      {
        ...
        "transportInfo": "이동수단 맞춤 팁 (뚜벅이면 역 정보, 자차면 주차 정보)",
    ...
    return NextResponse.json({ 
      ...
      doThis: [
        "근처 가장 힙한 푸드 마켓을 찾아 서로의 취향이 담긴 음식을 골라 가볍게 맛보기",
        "장소의 독특한 야경이나 오브제를 배경으로 오늘 이 순간의 우리를 기록하는 사진 남기기",
        "물소리가 들리는 근처 산책로를 따라 손을 잡고 걷으며 오늘 가장 행복했던 1분을 공유하기"
      ],
