...
[🚨 필수 제약 조건 및 기발함 가이드]
1. 뻔한 거 절대 금지: "카페 가기", "영화 보기", "반지 만들기", "향수 만들기", "파스타 먹기" 등 네이버 치면 나오는 식상한 코스는 무조건 감점.
2. 예산이 '공짜면 좋지'일 경우: 돈 한 푼 안 들이고도 할 수 있는 초현실적이고 기발한 것 (예: "각자 집에 있는 재료로 냉장고 파먹기 요리 대결", "도서관에서 서로에게 어울릴 만한 표지의 책 골라주고 첫 장만 읽어주기", "공원에 돗자리 펴고 막걸리 마시면서 이어폰 갈라 끼고 EDM 파티하기", "종이와 펜만 들고 서로 얼굴 안 보고 초상화 그려주기").
3. 만들기 태그가 있을 경우: 공방 예약하는 뻔한 만들기 말고, "다이소 천사점토로 서로의 최애 캐릭터 빚기", "서로의 안 입는 티셔츠 가위로 리폼해주기" 같은 일상적이고 엉뚱한 만들기 추천.
4. 디테일: 코스 이름만 던지지 말고, 왜 이 코스가 이 컨디션에 완벽한지 구체적이고 생생하게 묘사해. ${focusPoint}
