...
export async function POST(req: NextRequest) {
  try {
    ...
    const prompt = `
      ...
      [조건부 명령]
      1. 두 사람 모두 상태가 '녹초'라면: 반드시 '집 데이트', '실내 힐링', '호캉스' 등 이동이 거의 없고 편안하게 쉴 수 있는 실내 위주의 코스를 제안하세요. 
      2. 둘 중 한 명이라도 에너지가 있다면: 제공된 '현지 실시간 데이터'에 있는 실제 장소와 맛집 이름을 반드시 활용하여 3단계(Step-by-Step) 상세 동선을 짜주세요. 실제 지명과 상호명을 언급하여 신뢰도를 높여주세요.
